...
static int _get_name_user(char **name, char __user *name_user)
{
	long len = strlen_user(name_user), copied;
	if (len == 0) {
		return -EFAULT;
	}
	...
	*name = kmalloc(len, GFP_KERNEL);
	...
	copied = strncpy_from_user(*name, name_user, len);
